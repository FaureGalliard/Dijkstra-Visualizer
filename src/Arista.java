...
public class Arista {
    ...
    public Arista(Nodo nodoU, Nodo nodoV, int peso) {
        ...
        Line arrowLine1 = createArrowLine(-15);
        Line arrowLine2 = createArrowLine(15);
        this.edgeGroup = new Group(line, arrowLine1, arrowLine2);
    }
    ...
    private Line createArrowLine(double angleOffset) {
        ...
        double offsetFromMid = 0.85;
        double nodeRadiusOffset = nodoV.getRadius();
        ...
        DoubleBinding startX = new DoubleBinding() {
            {
                bind(nodoU.centerXProperty(), nodoV.centerXProperty());
            }
            @Override
            protected double computeValue() {
                ...
                return length == 0 ? nodoU.getCenterX() :
                        nodoU.getCenterX() + dx * (length - nodeRadiusOffset) * offsetFromMid / length;
            }
        };

        DoubleBinding startY = new DoubleBinding() {
            {
                bind(nodoU.centerYProperty(), nodoV.centerYProperty());
            }
            @Override
            protected double computeValue() {
                double dx = nodoV.getCenterX() - nodoU.getCenterX();
                ...
                return length == 0 ? nodoU.getCenterY() :
                        nodoU.getCenterY() + dy * (length - nodeRadiusOffset) * offsetFromMid / length;
            }
        };
